...
async function siteOrigin(): Promise<string> {
  ...
  // Sem a env var, NÃO cair no host da requisição: se o admin abriu o painel por
  // uma URL de deployment da Vercel (…-projects.vercel.app), o aluno receberia um
  // link protegido por Deployment Protection e veria "You Need Access".
  const h = await headers();
  const host = h.get("x-forwarded-host") ?? h.get("host");
  if (!host) return SITE_URL_PADRAO;
  if (host.endsWith(".vercel.app") && host !== new URL(SITE_URL_PADRAO).host) {
